...
% This example shows how to identify different colors in fabric by analyzing 
% the L*a*b* colorspace. The fabric image was acquired using the Image Acquisition 
% Toolbox™.
%% Step 1: Acquire Image
% Read in the |fabric.png| image, which is an image of colorful fabric. Instead 
% of using |fabric.png|, you can acquire an image using the following functions 
...
% layer 'a*' indicating where color falls along the red-green axis, and chromaticity 
% layer 'b*' indicating where the color falls along the blue-yellow axis.
% 
% Your approach is to choose a small sample region for each color and to calculate 
...
% that pixel and each color marker. The smallest distance will tell you that the 
% pixel most closely matches that color marker. For example, if the distance between 
% a pixel and the red color marker is the smallest, then the pixel would be labeled 
% as a red pixel.
% 
...
%% 
% Display the five segmented colors as a montage. Also display the background 
% pixels in the image that are not classified as a color. 
...
% into separate colors. For display purposes, label each point with its color 
% label.
